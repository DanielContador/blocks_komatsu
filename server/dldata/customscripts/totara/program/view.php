...
require_once($CFG->dirroot . '/theme/dlcourseflix/classes/output/totara_program_renderer.php');
...
//$PAGE->requires->css(new moodle_url('/theme/dlcourseflix/style/program.css'));
//$PAGE->requires->js(new moodle_url('/theme/dlcourseflix/javascript/program.js'));

$PAGE->requires->css(new moodle_url('/theme/dlcourseflix/style/program/globals.css'));
$PAGE->requires->css(new moodle_url('/theme/dlcourseflix/style/program/style.css'));
$PAGE->requires->css(new moodle_url('/theme/dlcourseflix/style/program/styleguide.css'));

$PAGE->set_program($program);
$PAGE->set_url('/totara/program/view.php', array('id' => $id, 'viewtype' => $viewtype));
$PAGE->set_pagelayout('program');
...
$PAGE->requires->strings_for_js(array('pleaseentervaliddate', 'pleaseentervalidreason', 'extensionrequest', 'cancel', 'ok'),
    'totara_program');
$PAGE->requires->strings_for_js(array('datepickerlongyeardisplayformat', 'datepickerlongyearplaceholder',
    'datepickerlongyearregexjs'), 'totara_core');
...
$args = array('args' => '{"id":' . $program->id . ', "userid":' . $USER->id . ', "user_fullname":' . json_encode(fullname($USER)) .
    ', "notify_html_fail":' . json_encode($notify_html_fail) . ', "notify_html":' . json_encode($notify_html) . '}');
$jsmodule = array(
...
if ($isadmin) {
    $PAGE->navbar->add(get_string('manageprograms', 'admin'),
        new moodle_url('/totara/program/manage.php', array('viewtype' => $viewtype)));
} else {
    $PAGE->navbar->add(get_string('findprograms', 'totara_program'),
        new moodle_url('/totara/program/index.php', array('viewtype' => $viewtype)));
}
...
echo $programrenderer->dl_display_program($program, $USER->id);

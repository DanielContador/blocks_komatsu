...
$PAGE->requires->css(new moodle_url('/theme/dlcourseflix/style/program/program_required.css'));
            ...
            foreach ($courseset_groups as $cs_group) {
                ...
                foreach ($cs_group as $courseset) {
                    switch ($courseset->contenttype) {
                        case program_content::CONTENTTYPE_COMPETENCY:
                            $courses = $courseset->get_competency_courses();
                            break;
                        case program_content::CONTENTTYPE_RECURRING:

...
$PAGE->requires->js(new moodle_url('/theme/dlcourseflix/style/mod_feedback/feedback.js'));
...
echo html_writer::start_tag('div', ['class' => 'dl_feedback_name']);
echo $OUTPUT->heading(format_string($feedback->name));
...
if ($feedbackcompletion->is_empty()) {
    ...
    if ($feedbackcompletion->just_completed()) {
        ...
        if (!$iswebview) { // Hide this button on webviews.
            $completeRenderer = $PAGE->get_renderer('core');
            $completeContext = new stdClass();
            $completeContext->feedbackname = format_string($feedback->name);
            echo $completeRenderer->render_from_template('theme_dlcourseflix/mod_feedback_complete', $completeContext);

...
$PAGE->set_heading($result->toctitle);
...
# put lateral menu in right panel
$scoe_menu = '';
if($PAGE->pagelayout == 'dlscormplayer')
{
    $bc = new block_contents();
    $bc->noheader = true;
    $bc->attributes['id'] = 'dl-scoes-lateral-menu';
    $bc->title = get_string('scoesmenutitle', 'scorm');

    $user = $USER;

    $toc_right = scorm_get_toc($user, $scorm, $cm->id, TOCFULLURL, $currentorg);

    $scoe_menu = $toc_right->toc;

    $PAGE->blocks->add_fake_block($bc, 'right-panel');
    // $scorm->hidetoc = SCORM_TOC_DISABLED;
...
echo $OUTPUT->flex_icon('theme_dlcourseflix|arrow-left', array('classes'=>'dl-show-scoes-menu-button d-sm-none'));
...
{
    if($scoe_menu) {
        echo html_writer::start_div('', array('id' => 'dl-scoes-responsive-menu', 'class'=> 'd-sm-none'));
        echo $scoe_menu;
        echo html_writer::end_div();
    }
    $PAGE->requires->js_call_amd('local_modextracontent/scormmodule','init', [$visible_sco]);
}

...
$PAGE->set_url($url);
$PAGE->requires->css(new moodle_url('/theme/dlcourseflix/style/mod_glossary/glossary.css'));

require_course_login($course->id, true, $cm);

$PAGE->set_pagelayout('dlglossary');
...
/// Search box
/*if ($showcommonelements) {
    echo '<form method="post" class="form form-inline mb-3" action="' . $CFG->wwwroot . '/mod/glossary/view.php">';

    echo html_writer::label(get_string('searchglossaryfor', 'mod_glossary'), 'mod_glossary_search', false,
        array('class' => 'sr-only'));
    if ($mode == 'search') {
        echo '<input id="mod_glossary_search" type="text" name="hook" size="20" value="' . s($hook) . '" alt="' . $strsearch .
            '" class="form-control mt-1"/> ';
    } else {
        echo '<input id="mod_glossary_search" type="text" name="hook" size="20" value="" alt="' . $strsearch .
            '" class="form-control mt-1"/> ';
    }
    echo '<input type="submit" value="' . $strsearch . '" name="searchbutton" class="btn btn-secondary mr-3 mt-1"/> ';
    if ($fullsearch || $mode != 'search') {
        $fullsearchchecked = 'checked="checked"';
    } else {
        $fullsearchchecked = '';
    }
    echo '<span class="checkbox"><label for="fullsearch" class="mt-1">';
...
if (has_capability('mod/glossary:write', $context) && $showcommonelements) {
    ...
    echo "<form class=\"form form-inline mb-3\" id=\"newentryform\" method=\"get\" action=\"$CFG->wwwroot/mod/glossary/edit.php\">";
    echo '<div>';

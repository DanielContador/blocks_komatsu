...
$course_completion = $DB->get_record('course_completions', array('course' => $course->id, 'userid' => $USER->id,));
...
// Modified by @rgonzalez@dl.cl 19/05/22
// Set has-sence class in body
$block_instance = $DB->get_record('block_instances', array('parentcontextid' => $context->id, 'blockname' => 'sence'));
if (!empty($block_instance)) {
    $PAGE->add_body_class('has-sence');
}
...
// Modified by @gsanchez@dl.cl 21/08/22
// Add modal if course has been completed via RPL
if ($course_completion)
    if ($course_completion->status == 75) {
        $PAGE->requires->js_amd_inline("
            require(['jquery', 'core/modal_factory','core/modal_events'], function($, ModalFactory,ModalEvents) {                
                ModalFactory.create({
                    body: '<h3>¡Atención! Este curso ya fue completado.</h3> <br> <p>Ya realizaste este curso y la información relacionada está en nuestros archivos.</p> <br> <p>No es necesario que vuelvas a realizarlo.</p>',

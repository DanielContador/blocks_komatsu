...
require_once('../local/advanced_password_recovery/locallib.php');
require_once('../local/advanced_password_recovery/forgot_password_form.php');
require_once('../login/set_password_form.php');
...
if (empty($token)) {
    ...
    loca_advanced_password_recovery_process_password_reset_request();
} else {
    ...
    if (!$tokeninsession && $_SERVER['REQUEST_METHOD'] === 'GET') {
        ...
        loca_advanced_password_recovery_process_password_set($token);
    }
}
